...
<body>
	...
	<c:out value="${ outData }" /> <br>		<%-- escapeXml 속성이 true(기본값) -> 출력데이터에 태그가 포함되어 있는 경우 해석되지 않음 --%>
	...
	<% } %>
	 ...
	 <%-- hi 변수에 저장된 값이 "안녕"일 경우 hihihi~ 출력, 그렇지 않은 경우 byebye 출력 --%>
	 ...
	 <%-- num1의 값이 20보다 크면 "20보다 큼", 10보다 크거나 같으면 "10보다 크거나 같음", 그렇지 않으면 "10보다 작음"을 출력 --%>
	 <%-- 
	 <% if(num1>20) { %>
	 	20보다 큼
	 <% } else { %>
	 ...
	 <c:choose>
		 ...
		 <c:when test="${ num1 ge 10 }">10보다 크거나 같음</c:when>
	 ...
	 <pre>
	 	...
	 	* 향상됨 for문 - (c:forEach var="변수명" items="배열/컬렉션(순차적으로 접근하고자 하는 객체)" 
