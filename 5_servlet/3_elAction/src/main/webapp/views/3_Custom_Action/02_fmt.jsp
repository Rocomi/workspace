...
<body>
	...
	<%-- n1, n2, n3 변수에 각각 123459789, 0.77, 50000이라는 값을 저장 --%>
	<c:set var="n1" value="123456789" />
	<c:set var="n2" value="0.77" />
	<c:set var="n3" value="50000" />
	
	* n1의 값을 그대로 출력 : <c:out value="${ n1 }" />, ${ n1 } <br>
	* 세자리마다 구분하여 출력 : <fmt:formatNumber value="${ n1 }"/> <br>
	<%-- groupingUsed 속성의 기본값 : true --%>
	* 그대로 출력 :<fmt:formatNumber value="${ n1 }" groupingUsed="false"/> <br>
	
	* type=percent : <fmt:formatNumber value="${ n2 }" type="percent"/> <br>
	* type=percent : <fmt:formatNumber value="${ n2 }" type="currency"/> <br>
	
	* type=percent : <fmt:formatNumber value="${ n3 }" type="currency"/> <br>
	...
	* 현재 날짜 출력 : ${ today } <br>
	<ul>
		...
		<li>
			medium 스타일(기본값): <fmt:formatDate value="${ today }" type="both" dateStyle="medium" timeStyle="medium"/>
		</li>
		<li>
