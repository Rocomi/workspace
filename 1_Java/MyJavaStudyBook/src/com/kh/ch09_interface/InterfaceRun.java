...
public class InterfaceRun {
	 ...
	 *  			자식 클래스마다 구현 내용ㅇ은 다르지만 동일한 형식을 적용( 메소드 통일성 / 표준화된 틀 제공)
	 *  
	 *  	=> 추상메소드가 없는경우도 추상클래스로 만들 수 있음
	 *  		-> 어떤경우 ? 해당 클래스가 미완성된 클래스라고 부여하고자 할 때
	 *  					객체 생성을 불가능하게 하고자 할 때
	 *  
	 *  * 상속(구현) 관계
	 *  	- 클래스에서 클래스를 상속받을 때 : 자식클래스 extends 부모클래스 (단일 상속만 가능) : 화살표 실선		: 화살표 실선
	 *  	- 클래스에서 인터페이스를 구현할 때 : 클래스 implements 인터페이스, 인터페이스, ... (다중 상속 가능) 	: 화살표 점선
	 *  	- 인터페이스에서 인터페이스를 상속받을 때 : 인터페이스 extends 인터페이스, 인터페이스, ... (다중상속 가능): 화살표 실선
	 *  
	 */

	public static void main(String[] args) {
		Plant monstera = new Monstera("카스테라");
		Plant lavender = new Lavender("다벤다");
