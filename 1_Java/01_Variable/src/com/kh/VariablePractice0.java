...
public class VariablePractice0 {
	...
	public static void main(String[] args) {
		...
		*/
		//변수를 사용하지 않은 경우
		System.out.println("[1] 현재 2024년이고, 올해 30살 입니다.");
		
		//변수 선언 및 대입
		int year;			//정수형 변수 year이라는 이름으로 선언
		year = 2024;		//year 변수에 2024라는 값을 대입
		
		int age = 30;		// 정수형 변수 age라는 이름으로 선언 및 30이라는 값을 대입
		...
		//----------------------------------------------------------------------------------------
		/*
		 *  * 형변환 : 연산을 할 때 자료셩의 종류가 다른 경우 형변환이 발생한다.
		 *   - 자동형변환 규칙
		 *   	[1] 값의 범위가 작은범위에서 큰 범위로 형변환 발생
		 *   	[2] 정수형과 실수형의 연산 시에는 정수형에서 실수형으로 형변환 발생
		 */
		int num1 = 10;
		double dNum1 = 5.5;
		
		//num1 변수가 자동 형변환이 발생 (int -> double)
		System.out.println(num1 + dNum1); // 결과 => 15.5
		
		System.out.println(num1 + (int)dNum1); // 결과 =>15
		
		// char <--> int 변환  ASCII(아스키코드)
		char ch = 'A';
		...
		System.out.println((char)(ch + 3)); // 결과 => 68
