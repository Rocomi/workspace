...
<body>
	...
	<div class="outer">
		...
		<form id="enroll-form" action="<%= contextPath %>/update.me" method="post">
			<table>
				...
				<tr>
					<td>전화번호</td>
					<td><input type="tel" name="phone" placeholder="- 포함해서 입력" value="${ loginUser.phone }"></td>
				</tr>
				<tr>
					<td>이메일</td>
				...
				<tr>
					<td>생년월일</td>
					<td><input type="text" name="birthday" placeholder="ex)990101" value="${ loginUser.birthday }"></td>
				</tr>
				<tr>
					<td>성별</td>
					<td>
	...
	<script>
		...
		$(function() {
			...
			const checkArr = $("input[name=gender]");

			for(let check of checkArr) {
				// check => js 방식으로 접근된 요소로 인식됨!
				if(gender.includes(check.value)){
