...
@RequestMapping("/member")	// 공통적으로 사용될 url주소
...
//	private MemberService mService = new MemberServiceImpl();
	...
	*/
	 ...
	 * 		전달되는 데이터의 키값을 받고자 하는 vo클래스의 필드명과 일치하도록 해줘야함.0
	 * 
	...
	public String insertMember(Member m, HttpSession session, Model model) {
		...
		// => 숫자 타입의 데이터 (나이)가 값이 없을 경우 400에러 발생 --> DB 처리 시 자동 형변환 되므로 
		//  			정수형 -> 문자열 형변환하여 처리
		
		// => 비밀번호 값을 입력된 값 그대로(평문) 저장하는 것이 아니라
		//		원래값을 알아보기 어렵게 만든 값(암호무)으로 저장할 것임!
